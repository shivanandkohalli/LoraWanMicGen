...
int gen_mic_keys(const unsigned char * nwk_key,const unsigned char * join_nonce,const unsigned char * join_eui,const unsigned char * dev_nonce, unsigned char *FNwkSIntKey, unsigned char *SNwkSIntKey)
{
    ...
    input[15] = 0x01;
    byte_copy(join_nonce, &input[14], SIZE_JOIN_NONCE);
    byte_copy(join_eui, &input[14-SIZE_JOIN_NONCE], SIZE_JOIN_EUI);
    byte_copy(dev_nonce, &input[14-SIZE_JOIN_NONCE-SIZE_JOIN_EUI], SIZE_DEV_NONCE);
    // Padding
    input[1] = 0;
    input[0] = 0;
